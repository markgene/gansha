...
message("# Given an input directory of FASTQs, create the command to merge the samples follow the convention https://wiki.stjude.org/display/CAB/St.+Jude+Hartwell+Center+GSF+fastq+file+directory+conventions")
...
data.frame(Fullpath = fastq_files) %>%
  ...
  dplyr::mutate(Sample = stringr::str_remove(Filename, "_S[0-9]+_L00[0-9]{1}_R[12]{1}_001.fastq.gz$")) %>%
