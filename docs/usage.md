# nf-core/gansha: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/gansha/usage](https://nf-co.re/gansha/usage)
...
```console
nextflow run nf-core/gansha --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/gansha releases page](https://github.com/nf-core/gansha/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
